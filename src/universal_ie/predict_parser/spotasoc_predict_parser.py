...
from ..structure_marker import (
    null_span,
    type_start,
    type_end,
    span_start,
)
from .predict_parser import PredictParser
from .utils import fix_unk_from_text
...
class SpotAsocPredictParser(PredictParser):
    ...
    def get_record_list(self, sel_tree, text=None, set_type='dev'):
        ...
        for spot_tree in sel_tree:
            ...
            if self.spot_set is not None and len(self.spot_set) > 0 and spot_type not in self.spot_set:
                continue
            ...
            for asoc_tree in spot_tree:
                ...
                if self.role_set is not None and len(self.role_set) > 0 and asoc_label not in self.role_set:
                    continue
                
                # Default UIE predict entity type together with relations, thus asoc_list use spot_type (entity type) instead of text
                # However, LLM project evaluate on single task per time only, thus use spot_text (entity text)
                # asoc_list += [(spot_type, asoc_label, asoc_text)]
                asoc_list += [(spot_text, asoc_label, asoc_text)]
