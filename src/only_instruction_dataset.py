...
class UIEInstructions(datasets.GeneratorBasedBuilder):
    ...
    def _load_dataset(self, dataset_path, labels_path = None):
        ...
        if self.config.sort_labels:
            if isinstance(instances, dict):
                instances = {k: sorted(v) for k, v in instances.items()}
                #sort the key
    ...
    def load_EMBED_INSTRUCTION_dataset(self, dataset_path, labels_path, dataset_name, sampling_strategy, max_num_instances, subset, base_path=None):
        instances, _ = self._load_dataset(dataset_path, labels_path=None)
        sample_template = {"Task": "EMBED_INSTRUCTION", "Dataset": dataset_name, "Samples": [], "subset": subset}
        
        instances = self._sampling_dataset(instances, sampling_strategy, max_num_instances)
        #instance format: { "task": "xxx", "dataset": "xxx"}
        for idx, instance in enumerate(instances):
            example = sample_template.copy()
            task = instance['task']
            dataset = instance['dataset']
            instruction = self._get_instruction(task)
            labels_path = os.path.join(base_path, task, dataset, 'labels.json')
            with open(labels_path, encoding="utf-8") as labels_f:
                labels = json.load(labels_f)
            if task == 'EET':
                labels_str = ', '.join(labels.keys())
                ...
                for event_type in labels:
                    labels_str = ', '.join(labels[event_type])
                    instruction += "Event type: " + event_type + " \n " + " Option: " + labels_str + " \n"
            elif task == 'RE':
                labels_str = ', '.join(labels)
                instruction = self._get_prompt(instruction, labels_str)
            elif task == 'NER':
            ...
            example["Instance"] = {
                "id": str(idx),
                "sentence": instance['sentence'] if 'sentence' in instance else None,
                "label": label,
                "ground_truth": label,
                "instruction": instruction,
                "answer_prefix": None,
    ...
    def _generate_examples(self, path=None, task_config=None, max_num_instances_per_task=None, subset=None):
        ...
        for task in task_config:
            ...
            elif task == 'EMBED_INSTRUCTION':
                load_func = self.load_EMBED_INSTRUCTION_dataset
            else:
                raise ValueError("Unsupport {} task, plz check {} task config!".format(task, subset))
