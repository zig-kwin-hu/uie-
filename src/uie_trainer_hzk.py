...
#modifiied by huzikun, when on_evaluate, pass the trainer and trial object to the callback function, so that it can modify the checkpoint saving process    
...
class UIETrainer(Seq2SeqTrainer):
    ...
    # modified by huzikun, rewrite the evaluation function, with customized call of evaluate, passing the trainer and trial object so that we can modify the save process.
